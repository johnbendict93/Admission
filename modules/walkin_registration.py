"""Module 03 — Walk-in Registration (live Supabase INSERT)"""
import streamlit as st
from datetime import date
from db import get_supabase
from config import (MAROON, GOLD, DEPARTMENTS, PROGRAMMES,
                    CATEGORIES, LEAD_SOURCES)
...
def show():
    ...
    st.markdown("""
    <div style='background:linear-gradient(90deg,#8B1A1A,#6E1414);
        ...
        <p style='color:#F5F0E8;margin:4px 0 0;font-size:0.9rem;'>
            Register a new walk-in enquiry — record is inserted live into Supabase.
        </p>
    ...
    with st.form("walkin_form", clear_on_submit=True):
        ...
        blood_group = c6.selectbox("Blood Group", [""] + BLOOD_GRP)
        ...
        address     = st.text_area("Address", height=68, placeholder="Street, City, District, Pincode")
        ...
        cd, ce = st.columns(2)
        hsc_percent = cd.number_input("12th / HSC %", min_value=0.0, max_value=100.0, step=0.1, value=None)
        ...
        notes       = st.text_area("Counselor Notes", height=80,
                                    placeholder="Anything noteworthy from the initial conversation")
    ...
    if submitted:
        ...
        if errors:
            ...
            try:
                ...
                st.caption("Tip: Check that the `applicants` table exists in Supabase and RLS is not blocking anonymous inserts.")
